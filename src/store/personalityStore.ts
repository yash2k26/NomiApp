...
interface PersonalityActions {
  ...
  generateDiaryEntry: (context: DiaryContext) => Promise<void>;
  markDiaryRead: () => void;
  ...
  /** Reset every personality field to defaults. Used by walletStore as part
   *  of the cascade-clear on disconnect so a new user doesn't inherit the
   *  prior user's traits, diary entries, dialogue, etc. AsyncStorage is
...
export const usePersonalityStore = create<PersonalityStore>((set, get) => ({
  ...
  memories: [],
  // Diary starts empty. We previously seeded two demo entries here, but they
  // contradicted the empty-state copy ("Nomi writes here while you are away")
  // and confused fresh users — making them think their pet had memories from
  // before they owned it. Real entries are now generated by tick() when the
  // user has been away for ≥2 hours.
  diaryEntries: [],
  lastDialogueAt: 0,
  ...
  generateDiaryEntry: async (ctx: DiaryContext) => {
    ...
    // Try LLM proxy first; fall back to local template if no endpoint or call fails.
    let text: string;
    let illustration: string;
    try {
      const { generateLLMDiaryEntry } = require('../lib/diaryGenerator');
      const llm = await generateLLMDiaryEntry(ctx, traits, memories);
      if (llm) {
        text = llm.text;
        illustration = llm.illustration;
      } else {
        const local = generateDiaryText(ctx, traits, memories);
        text = local.text;
    ...
    // Re-read diaryEntries in case other entries were added during the await.
    // Cap at 50 entries (was 7) — long-term users were silently losing their
    // pet's history. 50 is a sensible upper bound that keeps storage bounded
    // while making the diary feel persistent. Newest first.
    const latestEntries = get().diaryEntries;
    const updated = [entry, ...latestEntries].slice(0, 50);
  ...
  clearPersonality: () => {
    // Defaults must mirror the initial state declared at the top of create().
    // We deliberately drop the demo diary entries — they're meant to seed
    // first-launch wonder, not pollute a wallet-switched user's diary.
    set({
      traits: { playful: 20, foodie: 20, sleepy: 20, adventurous: 20, social: 20 },
      memories: [],
      diaryEntries: [],
